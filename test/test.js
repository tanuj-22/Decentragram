...
contract('Decentragram', ([deployer, author, tipper]) => {
  ...
  describe('images',async() => {
    ...
    it('create image', async() =>{
      ...
      // FAILURE TEST
      await decentragram.uploadImage('', 'Image Desc', { from: author }).should.be.rejected;

      await decentragram.uploadImage('Image hash', '', { from: author }).should.be.rejected;

      await decentragram.uploadImage('Image hash', 'Image Desc', { from: 0x0 }).should.be.rejected;

    })

    it('checks image from struct',async() => {
